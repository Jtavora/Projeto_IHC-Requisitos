...
from .CommonRouter import userRouter, oauth_scheme
from fastapi import Depends

user_controller = UserController()

@userRouter.post("/create_user", response_model=UserResponse)
def create_user(user: User, token: str = Depends(oauth_scheme)):
...
def get_user_by_username(username: str, token: str = Depends(oauth_scheme)):
...
def delete_user(user_id: str, token: str = Depends(oauth_scheme)):
...
def update_user(user_name: str, user: UserUpdate, token: str = Depends(oauth_scheme)):
